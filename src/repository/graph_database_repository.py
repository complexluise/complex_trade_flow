...
class GraphDatabaseRepository:
    ...
    def add_trade_relation(self, exporter_iso3: str, importer_iso3: str, product_category: str, year: int, value: float, quantity: float):
        exporter = Node("Country", iso3=exporter_iso3)
        importer = Node("Country", iso3=importer_iso3)
        relationship = Relationship(exporter, "EXPORTS_TO", importer, product_category=product_category, year=year, value=value, quantity=quantity)
